...
func DefaultPolicy() Policy {
	return Policy{MaxQuantity: 1000, AllowedTransitions: map[string]map[string]bool{StateIdle: {StateReserved: true}, StateReserved: {StateProcessing: true, StateIdle: true}, StateProcessing: {StateIdle: true}}}
}
func (p Policy) Validate(x Tool) error {
	if x.ID == "" || x.FabID == "" {
...
func (p Policy) Score(x Tool, now time.Time) Candidate {
	reasons := []string{}
