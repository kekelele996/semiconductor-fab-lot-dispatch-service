...
func (s *Service) Register(ctx context.Context, x Chamber) (Chamber, error) {
	if x.State == "" {
		x.State = StateAvailable
	}
	now := s.clock.Now()
	if x.ReadyAt.IsZero() {
		x.ReadyAt = now
	...
	if err := s.policy.Validate(x); err != nil {
		return Chamber{}, platform.Wrap("register", "chambers", x.ID, err)
	}
	saved, err := s.repo.Create(ctx, x)
	if err != nil {
		return Chamber{}, err
	}
	if err = s.events.Publish(ctx, platform.Event{Topic: "chambers.registered", Key: saved.ID, Version: saved.Version, At: now, Attributes: platform.CloneMap(saved.Constraints)}); err != nil {
		_ = s.repo.Delete(context.Background(), saved.ID, saved.Version)
		return Chamber{}, platform.Wrap("publish", "chambers", saved.ID, err)
	}
	return saved, nil
}
func (s *Service) Apply(ctx context.Context, cmd Command) (Chamber, error) {
	...
	if !s.policy.CanTransition(before.State, target) {
		return Chamber{}, platform.Wrap("transition", "chambers", cmd.ID, platform.ErrInvalidTransition)
	}
	updated, err := s.repo.Update(ctx, cmd.ID, cmd.ExpectedVersion, func(next *Chamber) error {
		next.State = target
		if cmd.Priority != 0 {
	...
	if err = s.events.Publish(ctx, platform.Event{Topic: "chambers.transitioned", Key: updated.ID, Version: updated.Version, At: updated.UpdatedAt, Attributes: attrs}); err != nil {
		_, rb := s.repo.Restore(context.Background(), before, updated.Version)
		if rb != nil {
			return Chamber{}, fmt.Errorf("publish: %w; rollback: %v", err, rb)
		}
		return Chamber{}, platform.Wrap("publish", "chambers", updated.ID, err)
	}
	return updated, nil
}
...
func (s *Service) ReserveBest(ctx context.Context, fab, target string) (Candidate, Chamber, error) {
	ranked, err := s.Candidates(ctx, fab)
	if err != nil {
		return Candidate{}, Chamber{}, err
	}
	for _, c := range ranked {
		...
		if ctx.Err() != nil {
			return Candidate{}, Chamber{}, ctx.Err()
		}
	}
	return Candidate{}, Chamber{}, platform.Wrap("reserve", "chambers", fab, platform.ErrCapacity)
}
