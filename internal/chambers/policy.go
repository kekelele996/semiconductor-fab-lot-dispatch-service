...
func DefaultPolicy() Policy {
	return Policy{MaxQuantity: 1000, AllowedTransitions: map[string]map[string]bool{StateAvailable: {StateConditioned: true}, StateConditioned: {StateOccupied: true, StateAvailable: true}, StateOccupied: {StateAvailable: true}}}
}
func (p Policy) Validate(x Chamber) error {
	if x.ID == "" || x.FabID == "" {
...
func (p Policy) Score(x Chamber, now time.Time) Candidate {
	reasons := []string{}
