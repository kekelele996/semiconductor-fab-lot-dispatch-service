...
func DefaultPolicy() Policy {
	return Policy{MaxQuantity: 1000, AllowedTransitions: map[string]map[string]bool{StateOpen: {StateReviewing: true}, StateReviewing: {StateReleased: true, StateOpen: true}, StateReleased: {StateOpen: true}}}
}
func (p Policy) Validate(x Hold) error {
	if x.ID == "" || x.FabID == "" {
...
func (p Policy) Score(x Hold, now time.Time) Candidate {
	reasons := []string{}
...
func Rank(p Policy, items []Hold, now time.Time) []Candidate {
