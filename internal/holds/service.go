...
func (s *Service) Register(ctx context.Context, x Hold) (Hold, error) {
	if x.State == "" {
		x.State = StateOpen
	}
	now := s.clock.Now()
	...
	if err := s.policy.Validate(x); err != nil {
		return Hold{}, platform.Wrap("register", "holds", x.ID, err)
	}
	saved, err := s.repo.Create(ctx, x)
	if err != nil {
		return Hold{}, err
	}
	if err = s.events.Publish(ctx, platform.Event{Topic: "holds.registered", Key: saved.ID, Version: saved.Version, At: now, Attributes: platform.CloneMap(saved.Constraints)}); err != nil {
		_ = s.repo.Delete(context.Background(), saved.ID, saved.Version)
		return Hold{}, platform.Wrap("publish", "holds", saved.ID, err)
	}
	return saved, nil
}
func (s *Service) Apply(ctx context.Context, cmd Command) (Hold, error) {
	...
	if !s.policy.CanTransition(before.State, target) {
		return Hold{}, platform.Wrap("transition", "holds", cmd.ID, platform.ErrInvalidTransition)
	}
	updated, err := s.repo.Update(ctx, cmd.ID, cmd.ExpectedVersion, func(next *Hold) error {
		next.State = target
		if cmd.Priority != 0 {
	...
	if err = s.events.Publish(ctx, platform.Event{Topic: "holds.transitioned", Key: updated.ID, Version: updated.Version, At: updated.UpdatedAt, Attributes: attrs}); err != nil {
		_, rb := s.repo.Restore(context.Background(), before, updated.Version)
		if rb != nil {
			return Hold{}, fmt.Errorf("publish: %w; rollback: %v", err, rb)
		}
		return Hold{}, platform.Wrap("publish", "holds", updated.ID, err)
	}
	return updated, nil
}
...
func (s *Service) ReserveBest(ctx context.Context, fab, target string) (Candidate, Hold, error) {
	ranked, err := s.Candidates(ctx, fab)
	if err != nil {
		return Candidate{}, Hold{}, err
	}
	for _, c := range ranked {
		...
		if ctx.Err() != nil {
			return Candidate{}, Hold{}, ctx.Err()
		}
	}
	return Candidate{}, Hold{}, platform.Wrap("reserve", "holds", fab, platform.ErrCapacity)
}
