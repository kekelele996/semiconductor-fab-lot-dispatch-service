...
func DefaultPolicy() Policy {
	return Policy{MaxQuantity: 1000, AllowedTransitions: map[string]map[string]bool{StateStored: {StateCheckedOut: true}, StateCheckedOut: {StateMounted: true, StateStored: true}, StateMounted: {StateStored: true}}}
}
func (p Policy) Validate(x Reticle) error {
	if x.ID == "" || x.FabID == "" {
...
func (p Policy) Score(x Reticle, now time.Time) Candidate {
	reasons := []string{}
