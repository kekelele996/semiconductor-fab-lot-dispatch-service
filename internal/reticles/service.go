...
func (s *Service) Register(ctx context.Context, x Reticle) (Reticle, error) {
	if x.State == "" {
		x.State = StateStored
	}
	now := s.clock.Now()
	...
	if err := s.policy.Validate(x); err != nil {
		return Reticle{}, platform.Wrap("register", "reticles", x.ID, err)
	}
	saved, err := s.repo.Create(ctx, x)
	if err != nil {
		return Reticle{}, err
	}
	if err = s.events.Publish(ctx, platform.Event{Topic: "reticles.registered", Key: saved.ID, Version: saved.Version, At: now, Attributes: platform.CloneMap(saved.Constraints)}); err != nil {
		_ = s.repo.Delete(context.Background(), saved.ID, saved.Version)
		return Reticle{}, platform.Wrap("publish", "reticles", saved.ID, err)
	}
	return saved, nil
}
func (s *Service) Apply(ctx context.Context, cmd Command) (Reticle, error) {
	...
	if !s.policy.CanTransition(before.State, target) {
		return Reticle{}, platform.Wrap("transition", "reticles", cmd.ID, platform.ErrInvalidTransition)
	}
	updated, err := s.repo.Update(ctx, cmd.ID, cmd.ExpectedVersion, func(next *Reticle) error {
		next.State = target
		if cmd.Priority != 0 {
	...
	if err = s.events.Publish(ctx, platform.Event{Topic: "reticles.transitioned", Key: updated.ID, Version: updated.Version, At: updated.UpdatedAt, Attributes: attrs}); err != nil {
		_, rb := s.repo.Restore(context.Background(), before, updated.Version)
		if rb != nil {
			return Reticle{}, fmt.Errorf("publish: %w; rollback: %v", err, rb)
		}
		return Reticle{}, platform.Wrap("publish", "reticles", updated.ID, err)
	}
	return updated, nil
}
...
func (s *Service) ReserveBest(ctx context.Context, fab, target string) (Candidate, Reticle, error) {
	ranked, err := s.Candidates(ctx, fab)
	if err != nil {
		return Candidate{}, Reticle{}, err
	}
	for _, c := range ranked {
		...
		if ctx.Err() != nil {
			return Candidate{}, Reticle{}, ctx.Err()
		}
	}
	return Candidate{}, Reticle{}, platform.Wrap("reserve", "reticles", fab, platform.ErrCapacity)
}
