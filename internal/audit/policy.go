...
func DefaultPolicy() Policy {
	return Policy{MaxQuantity: 1000, AllowedTransitions: map[string]map[string]bool{StateStaged: {StateCommitted: true}, StateCommitted: {StateArchived: true, StateStaged: true}, StateArchived: {StateStaged: true}}}
}
func (p Policy) Validate(x AuditRecord) error {
	if x.ID == "" || x.FabID == "" {
...
func (p Policy) Score(x AuditRecord, now time.Time) Candidate {
	reasons := []string{}
