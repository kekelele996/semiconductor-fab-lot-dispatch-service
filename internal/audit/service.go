...
func (s *Service) Register(ctx context.Context, x AuditRecord) (AuditRecord, error) {
	if x.State == "" {
		x.State = StateStaged
	}
	now := s.clock.Now()
	...
	if err := s.policy.Validate(x); err != nil {
		return AuditRecord{}, platform.Wrap("register", "audit", x.ID, err)
	}
	saved, err := s.repo.Create(ctx, x)
	if err != nil {
		return AuditRecord{}, err
	}
	if err = s.events.Publish(ctx, platform.Event{Topic: "audit.registered", Key: saved.ID, Version: saved.Version, At: now, Attributes: platform.CloneMap(saved.Constraints)}); err != nil {
		_ = s.repo.Delete(context.Background(), saved.ID, saved.Version)
		return AuditRecord{}, platform.Wrap("publish", "audit", saved.ID, err)
	}
	return saved, nil
}
func (s *Service) Apply(ctx context.Context, cmd Command) (AuditRecord, error) {
	...
	if !s.policy.CanTransition(before.State, target) {
		return AuditRecord{}, platform.Wrap("transition", "audit", cmd.ID, platform.ErrInvalidTransition)
	}
	updated, err := s.repo.Update(ctx, cmd.ID, cmd.ExpectedVersion, func(next *AuditRecord) error {
		next.State = target
		if cmd.Priority != 0 {
	...
	if err = s.events.Publish(ctx, platform.Event{Topic: "audit.transitioned", Key: updated.ID, Version: updated.Version, At: updated.UpdatedAt, Attributes: attrs}); err != nil {
		_, rb := s.repo.Restore(context.Background(), before, updated.Version)
		if rb != nil {
			return AuditRecord{}, fmt.Errorf("publish: %w; rollback: %v", err, rb)
		}
		return AuditRecord{}, platform.Wrap("publish", "audit", updated.ID, err)
	}
	return updated, nil
}
...
func (s *Service) ReserveBest(ctx context.Context, fab, target string) (Candidate, AuditRecord, error) {
	ranked, err := s.Candidates(ctx, fab)
	if err != nil {
		return Candidate{}, AuditRecord{}, err
	}
	for _, c := range ranked {
		...
		if ctx.Err() != nil {
			return Candidate{}, AuditRecord{}, ctx.Err()
		}
	}
	return Candidate{}, AuditRecord{}, platform.Wrap("reserve", "audit", fab, platform.ErrCapacity)
}
