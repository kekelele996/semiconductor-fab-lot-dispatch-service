...
func DefaultPolicy() Policy {
	return Policy{MaxQuantity: 1000, AllowedTransitions: map[string]map[string]bool{StateRequested: {StateRunning: true}, StateRunning: {StateVerified: true, StateRequested: true}, StateVerified: {StateRequested: true}}}
}
func (p Policy) Validate(x PurgeCycle) error {
	if x.ID == "" || x.FabID == "" {
...
func (p Policy) Score(x PurgeCycle, now time.Time) Candidate {
	reasons := []string{}
