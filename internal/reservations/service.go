...
func (s *Service) Register(ctx context.Context, x Reservation) (Reservation, error) {
	if x.State == "" {
		x.State = StateProposed
	}
	now := s.clock.Now()
	if x.ReadyAt.IsZero() {
		x.ReadyAt = now
	...
	if err := s.policy.Validate(x); err != nil {
		return Reservation{}, platform.Wrap("register", "reservations", x.ID, err)
	}
	saved, err := s.repo.Create(ctx, x)
	if err != nil {
		return Reservation{}, err
	}
	if err = s.events.Publish(ctx, platform.Event{Topic: "reservations.registered", Key: saved.ID, Version: saved.Version, At: now, Attributes: platform.CloneMap(saved.Constraints)}); err != nil {
		_ = s.repo.Delete(context.Background(), saved.ID, saved.Version)
		return Reservation{}, platform.Wrap("publish", "reservations", saved.ID, err)
	}
	return saved, nil
}
func (s *Service) Apply(ctx context.Context, cmd Command) (Reservation, error) {
	...
	if !s.policy.CanTransition(before.State, target) {
		return Reservation{}, platform.Wrap("transition", "reservations", cmd.ID, platform.ErrInvalidTransition)
	}
	updated, err := s.repo.Update(ctx, cmd.ID, cmd.ExpectedVersion, func(next *Reservation) error {
		next.State = target
		if cmd.Priority != 0 {
	...
	if err = s.events.Publish(ctx, platform.Event{Topic: "reservations.transitioned", Key: updated.ID, Version: updated.Version, At: updated.UpdatedAt, Attributes: attrs}); err != nil {
		_, rb := s.repo.Restore(context.Background(), before, updated.Version)
		if rb != nil {
			return Reservation{}, fmt.Errorf("publish: %w; rollback: %v", err, rb)
		}
		return Reservation{}, platform.Wrap("publish", "reservations", updated.ID, err)
	}
	return updated, nil
}
...
func (s *Service) ReserveBest(ctx context.Context, fab, target string) (Candidate, Reservation, error) {
	ranked, err := s.Candidates(ctx, fab)
	if err != nil {
		return Candidate{}, Reservation{}, err
	}
	for _, c := range ranked {
		...
		if ctx.Err() != nil {
			return Candidate{}, Reservation{}, ctx.Err()
		}
	}
	return Candidate{}, Reservation{}, platform.Wrap("reserve", "reservations", fab, platform.ErrCapacity)
}
