...
func DefaultPolicy() Policy {
	return Policy{MaxQuantity: 1000, AllowedTransitions: map[string]map[string]bool{StateProposed: {StateHeld: true}, StateHeld: {StateConsumed: true, StateProposed: true}, StateConsumed: {StateProposed: true}}}
}
func (p Policy) Validate(x Reservation) error {
	if x.ID == "" || x.FabID == "" {
...
func (p Policy) Score(x Reservation, now time.Time) Candidate {
	reasons := []string{}
...
func Rank(p Policy, items []Reservation, now time.Time) []Candidate {
