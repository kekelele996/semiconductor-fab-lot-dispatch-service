...
func DefaultPolicy() Policy {
	return Policy{MaxQuantity: 1000, AllowedTransitions: map[string]map[string]bool{StateDetected: {StateExecuting: true}, StateExecuting: {StateRestored: true, StateDetected: true}, StateRestored: {StateDetected: true}}}
}
func (p Policy) Validate(x RecoveryPlan) error {
	if x.ID == "" || x.FabID == "" {
...
func (p Policy) Score(x RecoveryPlan, now time.Time) Candidate {
	reasons := []string{}
