...
func DefaultPolicy() Policy {
	return Policy{MaxQuantity: 1000, AllowedTransitions: map[string]map[string]bool{StateRequested: {StateApproved: true}, StateApproved: {StateRouted: true, StateRequested: true}, StateRouted: {StateRequested: true}}}
}
func (p Policy) Validate(x ReworkPlan) error {
	if x.ID == "" || x.FabID == "" {
...
func (p Policy) Score(x ReworkPlan, now time.Time) Candidate {
	reasons := []string{}
