...
func DefaultPolicy() Policy {
	return Policy{MaxQuantity: 1000, AllowedTransitions: map[string]map[string]bool{StateDraft: {StateArmed: true}, StateArmed: {StateCollected: true, StateDraft: true}, StateCollected: {StateDraft: true}}}
}
func (p Policy) Validate(x SamplePlan) error {
	if x.ID == "" || x.FabID == "" {
...
func (p Policy) Score(x SamplePlan, now time.Time) Candidate {
	reasons := []string{}
