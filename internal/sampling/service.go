...
func (s *Service) Register(ctx context.Context, x SamplePlan) (SamplePlan, error) {
	if x.State == "" {
		x.State = StateDraft
	}
	now := s.clock.Now()
	...
	if err := s.policy.Validate(x); err != nil {
		return SamplePlan{}, platform.Wrap("register", "sampling", x.ID, err)
	}
	saved, err := s.repo.Create(ctx, x)
	if err != nil {
		return SamplePlan{}, err
	}
	if err = s.events.Publish(ctx, platform.Event{Topic: "sampling.registered", Key: saved.ID, Version: saved.Version, At: now, Attributes: platform.CloneMap(saved.Constraints)}); err != nil {
		_ = s.repo.Delete(context.Background(), saved.ID, saved.Version)
		return SamplePlan{}, platform.Wrap("publish", "sampling", saved.ID, err)
	}
	return saved, nil
}
func (s *Service) Apply(ctx context.Context, cmd Command) (SamplePlan, error) {
	...
	if !s.policy.CanTransition(before.State, target) {
		return SamplePlan{}, platform.Wrap("transition", "sampling", cmd.ID, platform.ErrInvalidTransition)
	}
	updated, err := s.repo.Update(ctx, cmd.ID, cmd.ExpectedVersion, func(next *SamplePlan) error {
		next.State = target
		if cmd.Priority != 0 {
	...
	if err = s.events.Publish(ctx, platform.Event{Topic: "sampling.transitioned", Key: updated.ID, Version: updated.Version, At: updated.UpdatedAt, Attributes: attrs}); err != nil {
		_, rb := s.repo.Restore(context.Background(), before, updated.Version)
		if rb != nil {
			return SamplePlan{}, fmt.Errorf("publish: %w; rollback: %v", err, rb)
		}
		return SamplePlan{}, platform.Wrap("publish", "sampling", updated.ID, err)
	}
	return updated, nil
}
...
func (s *Service) ReserveBest(ctx context.Context, fab, target string) (Candidate, SamplePlan, error) {
	ranked, err := s.Candidates(ctx, fab)
	if err != nil {
		return Candidate{}, SamplePlan{}, err
	}
	for _, c := range ranked {
		...
		if ctx.Err() != nil {
			return Candidate{}, SamplePlan{}, ctx.Err()
		}
	}
	return Candidate{}, SamplePlan{}, platform.Wrap("reserve", "sampling", fab, platform.ErrCapacity)
}
