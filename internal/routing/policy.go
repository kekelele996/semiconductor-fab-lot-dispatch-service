...
func DefaultPolicy() Policy {
	return Policy{MaxQuantity: 1000, AllowedTransitions: map[string]map[string]bool{StateDraft: {StateValidated: true}, StateValidated: {StateActive: true, StateDraft: true}, StateActive: {StateDraft: true}}}
}
func (p Policy) Validate(x Route) error {
	if x.ID == "" || x.FabID == "" {
...
func (p Policy) Score(x Route, now time.Time) Candidate {
	reasons := []string{}
...
func Rank(p Policy, items []Route, now time.Time) []Candidate {
