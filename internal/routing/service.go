...
func (s *Service) Register(ctx context.Context, x Route) (Route, error) {
	if x.State == "" {
		x.State = StateDraft
	}
	now := s.clock.Now()
	...
	if err := s.policy.Validate(x); err != nil {
		return Route{}, platform.Wrap("register", "routing", x.ID, err)
	}
	saved, err := s.repo.Create(ctx, x)
	if err != nil {
		return Route{}, err
	}
	if err = s.events.Publish(ctx, platform.Event{Topic: "routing.registered", Key: saved.ID, Version: saved.Version, At: now, Attributes: platform.CloneMap(saved.Constraints)}); err != nil {
		_ = s.repo.Delete(context.Background(), saved.ID, saved.Version)
		return Route{}, platform.Wrap("publish", "routing", saved.ID, err)
	}
	return saved, nil
}
func (s *Service) Apply(ctx context.Context, cmd Command) (Route, error) {
	...
	if !s.policy.CanTransition(before.State, target) {
		return Route{}, platform.Wrap("transition", "routing", cmd.ID, platform.ErrInvalidTransition)
	}
	updated, err := s.repo.Update(ctx, cmd.ID, cmd.ExpectedVersion, func(next *Route) error {
		next.State = target
		if cmd.Priority != 0 {
	...
	if err = s.events.Publish(ctx, platform.Event{Topic: "routing.transitioned", Key: updated.ID, Version: updated.Version, At: updated.UpdatedAt, Attributes: attrs}); err != nil {
		_, rb := s.repo.Restore(context.Background(), before, updated.Version)
		if rb != nil {
			return Route{}, fmt.Errorf("publish: %w; rollback: %v", err, rb)
		}
		return Route{}, platform.Wrap("publish", "routing", updated.ID, err)
	}
	return updated, nil
}
...
func (s *Service) ReserveBest(ctx context.Context, fab, target string) (Candidate, Route, error) {
	...
	for _, c := range ranked {
		...
		if ctx.Err() != nil {
			return Candidate{}, Route{}, ctx.Err()
		}
	}
	return Candidate{}, Route{}, platform.Wrap("reserve", "routing", fab, platform.ErrCapacity)
}
