...
func DefaultPolicy() Policy {
	return Policy{MaxQuantity: 1000, AllowedTransitions: map[string]map[string]bool{StatePrepared: {StateAccepted: true}, StateAccepted: {StateClosed: true, StatePrepared: true}, StateClosed: {StatePrepared: true}}}
}
func (p Policy) Validate(x ShiftHandoff) error {
	if x.ID == "" || x.FabID == "" {
...
func (p Policy) Score(x ShiftHandoff, now time.Time) Candidate {
	reasons := []string{}
...
func Rank(p Policy, items []ShiftHandoff, now time.Time) []Candidate {
