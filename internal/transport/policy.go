...
func DefaultPolicy() Policy {
	return Policy{MaxQuantity: 1000, AllowedTransitions: map[string]map[string]bool{StateRequested: {StateAssigned: true}, StateAssigned: {StateDelivered: true, StateRequested: true}, StateDelivered: {StateRequested: true}}}
}
func (p Policy) Validate(x Move) error {
	if x.ID == "" || x.FabID == "" {
...
func (p Policy) Score(x Move, now time.Time) Candidate {
	reasons := []string{}
