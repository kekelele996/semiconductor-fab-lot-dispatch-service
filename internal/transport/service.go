...
func (s *Service) Register(ctx context.Context, x Move) (Move, error) {
	if x.State == "" {
	...
	if err := s.policy.Validate(x); err != nil {
		return Move{}, platform.Wrap("register", "transport", x.ID, err)
	}
	saved, err := s.repo.Create(ctx, x)
	if err != nil {
		return Move{}, err
	}
	if err = s.events.Publish(ctx, platform.Event{Topic: "transport.registered", Key: saved.ID, Version: saved.Version, At: now, Attributes: platform.CloneMap(saved.Constraints)}); err != nil {
		_ = s.repo.Delete(context.Background(), saved.ID, saved.Version)
		return Move{}, platform.Wrap("publish", "transport", saved.ID, err)
	}
	return saved, nil
}
func (s *Service) Apply(ctx context.Context, cmd Command) (Move, error) {
	...
	if !s.policy.CanTransition(before.State, target) {
		return Move{}, platform.Wrap("transition", "transport", cmd.ID, platform.ErrInvalidTransition)
	}
	updated, err := s.repo.Update(ctx, cmd.ID, cmd.ExpectedVersion, func(next *Move) error {
		next.State = target
		if cmd.Priority != 0 {
	...
	if err = s.events.Publish(ctx, platform.Event{Topic: "transport.transitioned", Key: updated.ID, Version: updated.Version, At: updated.UpdatedAt, Attributes: attrs}); err != nil {
		_, rb := s.repo.Restore(context.Background(), before, updated.Version)
		if rb != nil {
			return Move{}, fmt.Errorf("publish: %w; rollback: %v", err, rb)
		}
		return Move{}, platform.Wrap("publish", "transport", updated.ID, err)
	}
	return updated, nil
}
...
func (s *Service) ReserveBest(ctx context.Context, fab, target string) (Candidate, Move, error) {
	ranked, err := s.Candidates(ctx, fab)
	if err != nil {
		return Candidate{}, Move{}, err
	}
	for _, c := range ranked {
		...
		if ctx.Err() != nil {
			return Candidate{}, Move{}, ctx.Err()
		}
	}
	return Candidate{}, Move{}, platform.Wrap("reserve", "transport", fab, platform.ErrCapacity)
}
