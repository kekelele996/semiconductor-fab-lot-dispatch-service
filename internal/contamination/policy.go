...
func DefaultPolicy() Policy {
	return Policy{MaxQuantity: 1000, AllowedTransitions: map[string]map[string]bool{StateClean: {StateRestricted: true}, StateRestricted: {StateQuarantined: true, StateClean: true}, StateQuarantined: {StateClean: true}}}
}
func (p Policy) Validate(x ZoneState) error {
	if x.ID == "" || x.FabID == "" {
...
func (p Policy) Score(x ZoneState, now time.Time) Candidate {
	reasons := []string{}
