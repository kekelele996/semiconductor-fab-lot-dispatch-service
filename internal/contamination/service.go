...
func (s *Service) Register(ctx context.Context, x ZoneState) (ZoneState, error) {
	if x.State == "" {
		x.State = StateClean
	}
	now := s.clock.Now()
	...
	if err := s.policy.Validate(x); err != nil {
		return ZoneState{}, platform.Wrap("register", "contamination", x.ID, err)
	}
	saved, err := s.repo.Create(ctx, x)
	if err != nil {
		return ZoneState{}, err
	}
	if err = s.events.Publish(ctx, platform.Event{Topic: "contamination.registered", Key: saved.ID, Version: saved.Version, At: now, Attributes: platform.CloneMap(saved.Constraints)}); err != nil {
		_ = s.repo.Delete(context.Background(), saved.ID, saved.Version)
		return ZoneState{}, platform.Wrap("publish", "contamination", saved.ID, err)
	}
	return saved, nil
}
func (s *Service) Apply(ctx context.Context, cmd Command) (ZoneState, error) {
	...
	if !s.policy.CanTransition(before.State, target) {
		return ZoneState{}, platform.Wrap("transition", "contamination", cmd.ID, platform.ErrInvalidTransition)
	}
	updated, err := s.repo.Update(ctx, cmd.ID, cmd.ExpectedVersion, func(next *ZoneState) error {
		next.State = target
		if cmd.Priority != 0 {
	...
	if err = s.events.Publish(ctx, platform.Event{Topic: "contamination.transitioned", Key: updated.ID, Version: updated.Version, At: updated.UpdatedAt, Attributes: attrs}); err != nil {
		_, rb := s.repo.Restore(context.Background(), before, updated.Version)
		if rb != nil {
			return ZoneState{}, fmt.Errorf("publish: %w; rollback: %v", err, rb)
		}
		return ZoneState{}, platform.Wrap("publish", "contamination", updated.ID, err)
	}
	return updated, nil
}
...
func (s *Service) ReserveBest(ctx context.Context, fab, target string) (Candidate, ZoneState, error) {
	ranked, err := s.Candidates(ctx, fab)
	if err != nil {
		return Candidate{}, ZoneState{}, err
	}
	for _, c := range ranked {
		...
		if ctx.Err() != nil {
			return Candidate{}, ZoneState{}, ctx.Err()
		}
	}
	return Candidate{}, ZoneState{}, platform.Wrap("reserve", "contamination", fab, platform.ErrCapacity)
}
