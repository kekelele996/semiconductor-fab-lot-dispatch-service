...
func DefaultPolicy() Policy {
	return Policy{MaxQuantity: 1000, AllowedTransitions: map[string]map[string]bool{StateWaiting: {StateSelected: true}, StateSelected: {StateStarted: true, StateWaiting: true}, StateStarted: {StateWaiting: true}}}
}
func (p Policy) Validate(x QueueEntry) error {
	if x.ID == "" || x.FabID == "" {
...
func (p Policy) Score(x QueueEntry, now time.Time) Candidate {
	reasons := []string{}
