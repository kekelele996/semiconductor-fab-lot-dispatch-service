...
func DefaultPolicy() Policy {
	return Policy{MaxQuantity: 1000, AllowedTransitions: map[string]map[string]bool{StateForming: {StateSealed: true}, StateSealed: {StateLaunched: true, StateForming: true}, StateLaunched: {StateForming: true}}}
}
func (p Policy) Validate(x Batch) error {
	if x.ID == "" || x.FabID == "" {
...
func (p Policy) Score(x Batch, now time.Time) Candidate {
	reasons := []string{}
