...
func DefaultPolicy() Policy {
	return Policy{MaxQuantity: 1000, AllowedTransitions: map[string]map[string]bool{StateForecast: {StateLimited: true}, StateLimited: {StateSettled: true, StateForecast: true}, StateSettled: {StateForecast: true}}}
}
func (p Policy) Validate(x EnergyWindow) error {
	if x.ID == "" || x.FabID == "" {
...
func (p Policy) Score(x EnergyWindow, now time.Time) Candidate {
	reasons := []string{}
