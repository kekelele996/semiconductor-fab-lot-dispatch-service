...
func DefaultPolicy() Policy {
	return Policy{MaxQuantity: 1000, AllowedTransitions: map[string]map[string]bool{StatePending: {StateCommitted: true}, StateCommitted: {StateExecuted: true, StatePending: true}, StateExecuted: {StatePending: true}}}
}
func (p Policy) Validate(x Decision) error {
	if x.ID == "" || x.FabID == "" {
...
func (p Policy) Score(x Decision, now time.Time) Candidate {
	reasons := []string{}
