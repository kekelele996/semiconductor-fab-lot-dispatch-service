...
func (s *Service) Register(ctx context.Context, x Decision) (Decision, error) {
	if x.State == "" {
	...
	if err := s.policy.Validate(x); err != nil {
		return Decision{}, platform.Wrap("register", "dispatch", x.ID, err)
	}
	saved, err := s.repo.Create(ctx, x)
	if err != nil {
		return Decision{}, err
	}
	if err = s.events.Publish(ctx, platform.Event{Topic: "dispatch.registered", Key: saved.ID, Version: saved.Version, At: now, Attributes: platform.CloneMap(saved.Constraints)}); err != nil {
		_ = s.repo.Delete(context.Background(), saved.ID, saved.Version)
		return Decision{}, platform.Wrap("publish", "dispatch", saved.ID, err)
	}
	return saved, nil
}
func (s *Service) Apply(ctx context.Context, cmd Command) (Decision, error) {
	...
	if !s.policy.CanTransition(before.State, target) {
		return Decision{}, platform.Wrap("transition", "dispatch", cmd.ID, platform.ErrInvalidTransition)
	}
	updated, err := s.repo.Update(ctx, cmd.ID, cmd.ExpectedVersion, func(next *Decision) error {
		next.State = target
		if cmd.Priority != 0 {
	...
	if err = s.events.Publish(ctx, platform.Event{Topic: "dispatch.transitioned", Key: updated.ID, Version: updated.Version, At: updated.UpdatedAt, Attributes: attrs}); err != nil {
		_, rb := s.repo.Restore(context.Background(), before, updated.Version)
		if rb != nil {
			return Decision{}, fmt.Errorf("publish: %w; rollback: %v", err, rb)
		}
		return Decision{}, platform.Wrap("publish", "dispatch", updated.ID, err)
	}
	return updated, nil
}
...
func (s *Service) ReserveBest(ctx context.Context, fab, target string) (Candidate, Decision, error) {
	...
	for _, c := range ranked {
		...
		if ctx.Err() != nil {
			return Candidate{}, Decision{}, ctx.Err()
		}
	}
	return Candidate{}, Decision{}, platform.Wrap("reserve", "dispatch", fab, platform.ErrCapacity)
}
