...
func DefaultPolicy() Policy {
	return Policy{MaxQuantity: 1000, AllowedTransitions: map[string]map[string]bool{StateDraft: {StateQualified: true}, StateQualified: {StateReleased: true, StateDraft: true}, StateReleased: {StateDraft: true}}}
}
func (p Policy) Validate(x Recipe) error {
	if x.ID == "" || x.FabID == "" {
...
func (p Policy) Score(x Recipe, now time.Time) Candidate {
	reasons := []string{}
