...
func (s *Service) Register(ctx context.Context, x Recipe) (Recipe, error) {
	if x.State == "" {
		x.State = StateDraft
	}
	now := s.clock.Now()
	...
	if err := s.policy.Validate(x); err != nil {
		return Recipe{}, platform.Wrap("register", "recipes", x.ID, err)
	}
	saved, err := s.repo.Create(ctx, x)
	if err != nil {
		return Recipe{}, err
	}
	if err = s.events.Publish(ctx, platform.Event{Topic: "recipes.registered", Key: saved.ID, Version: saved.Version, At: now, Attributes: platform.CloneMap(saved.Constraints)}); err != nil {
		_ = s.repo.Delete(context.Background(), saved.ID, saved.Version)
		return Recipe{}, platform.Wrap("publish", "recipes", saved.ID, err)
	}
	return saved, nil
}
func (s *Service) Apply(ctx context.Context, cmd Command) (Recipe, error) {
	...
	if !s.policy.CanTransition(before.State, target) {
		return Recipe{}, platform.Wrap("transition", "recipes", cmd.ID, platform.ErrInvalidTransition)
	}
	updated, err := s.repo.Update(ctx, cmd.ID, cmd.ExpectedVersion, func(next *Recipe) error {
		next.State = target
		if cmd.Priority != 0 {
	...
	if err = s.events.Publish(ctx, platform.Event{Topic: "recipes.transitioned", Key: updated.ID, Version: updated.Version, At: updated.UpdatedAt, Attributes: attrs}); err != nil {
		_, rb := s.repo.Restore(context.Background(), before, updated.Version)
		if rb != nil {
			return Recipe{}, fmt.Errorf("publish: %w; rollback: %v", err, rb)
		}
		return Recipe{}, platform.Wrap("publish", "recipes", updated.ID, err)
	}
	return updated, nil
}
...
func (s *Service) ReserveBest(ctx context.Context, fab, target string) (Candidate, Recipe, error) {
	ranked, err := s.Candidates(ctx, fab)
	if err != nil {
		return Candidate{}, Recipe{}, err
	}
	for _, c := range ranked {
		...
		if ctx.Err() != nil {
			return Candidate{}, Recipe{}, ctx.Err()
		}
	}
	return Candidate{}, Recipe{}, platform.Wrap("reserve", "recipes", fab, platform.ErrCapacity)
}
