...
func DefaultPolicy() Policy {
	return Policy{MaxQuantity: 1000, AllowedTransitions: map[string]map[string]bool{StateQueued: {StateRunning: true}, StateRunning: {StateCompleted: true, StateQueued: true}, StateCompleted: {StateQueued: true}}}
}
func (p Policy) Validate(x Lot) error {
	if x.ID == "" || x.FabID == "" {
...
func (p Policy) Score(x Lot, now time.Time) Candidate {
	reasons := []string{}
...
func Rank(p Policy, items []Lot, now time.Time) []Candidate {
