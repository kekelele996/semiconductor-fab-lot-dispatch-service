...
func DefaultPolicy() Policy {
	return Policy{MaxQuantity: 1000, AllowedTransitions: map[string]map[string]bool{StatePlanned: {StateActive: true}, StateActive: {StateClosed: true, StatePlanned: true}, StateClosed: {StatePlanned: true}}}
}
func (p Policy) Validate(x WorkWindow) error {
	if x.ID == "" || x.FabID == "" {
...
func (p Policy) Score(x WorkWindow, now time.Time) Candidate {
	reasons := []string{}
...
func Rank(p Policy, items []WorkWindow, now time.Time) []Candidate {
