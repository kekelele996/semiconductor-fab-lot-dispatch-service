...
func (s *Service) Register(ctx context.Context, x WorkWindow) (WorkWindow, error) {
	if x.State == "" {
		x.State = StatePlanned
	}
	now := s.clock.Now()
	if x.ReadyAt.IsZero() {
		x.ReadyAt = now
	...
	if err := s.policy.Validate(x); err != nil {
		return WorkWindow{}, platform.Wrap("register", "maintenance", x.ID, err)
	}
	saved, err := s.repo.Create(ctx, x)
	if err != nil {
		return WorkWindow{}, err
	}
	if err = s.events.Publish(ctx, platform.Event{Topic: "maintenance.registered", Key: saved.ID, Version: saved.Version, At: now, Attributes: platform.CloneMap(saved.Constraints)}); err != nil {
		_ = s.repo.Delete(context.Background(), saved.ID, saved.Version)
		return WorkWindow{}, platform.Wrap("publish", "maintenance", saved.ID, err)
	}
	return saved, nil
}
func (s *Service) Apply(ctx context.Context, cmd Command) (WorkWindow, error) {
	...
	if !s.policy.CanTransition(before.State, target) {
		return WorkWindow{}, platform.Wrap("transition", "maintenance", cmd.ID, platform.ErrInvalidTransition)
	}
	updated, err := s.repo.Update(ctx, cmd.ID, cmd.ExpectedVersion, func(next *WorkWindow) error {
		next.State = target
		if cmd.Priority != 0 {
	...
	if err = s.events.Publish(ctx, platform.Event{Topic: "maintenance.transitioned", Key: updated.ID, Version: updated.Version, At: updated.UpdatedAt, Attributes: attrs}); err != nil {
		_, rb := s.repo.Restore(context.Background(), before, updated.Version)
		if rb != nil {
			return WorkWindow{}, fmt.Errorf("publish: %w; rollback: %v", err, rb)
		}
		return WorkWindow{}, platform.Wrap("publish", "maintenance", updated.ID, err)
	}
	return updated, nil
}
...
func (s *Service) ReserveBest(ctx context.Context, fab, target string) (Candidate, WorkWindow, error) {
	ranked, err := s.Candidates(ctx, fab)
	if err != nil {
		return Candidate{}, WorkWindow{}, err
	}
	for _, c := range ranked {
		...
		if ctx.Err() != nil {
			return Candidate{}, WorkWindow{}, ctx.Err()
		}
	}
	return Candidate{}, WorkWindow{}, platform.Wrap("reserve", "maintenance", fab, platform.ErrCapacity)
}
