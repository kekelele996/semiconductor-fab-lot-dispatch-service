...
func DefaultPolicy() Policy {
	return Policy{MaxQuantity: 1000, AllowedTransitions: map[string]map[string]bool{StateEmpty: {StateLoaded: true}, StateLoaded: {StateSealed: true, StateEmpty: true}, StateSealed: {StateEmpty: true}}}
}
func (p Policy) Validate(x Carrier) error {
	if x.ID == "" || x.FabID == "" {
...
func (p Policy) Score(x Carrier, now time.Time) Candidate {
	reasons := []string{}
