...
func DefaultPolicy() Policy {
	return Policy{MaxQuantity: 1000, AllowedTransitions: map[string]map[string]bool{StateScheduled: {StateRunning: true}, StateRunning: {StatePassed: true, StateScheduled: true}, StatePassed: {StateScheduled: true}}}
}
func (p Policy) Validate(x Qualification) error {
	if x.ID == "" || x.FabID == "" {
...
func (p Policy) Score(x Qualification, now time.Time) Candidate {
	reasons := []string{}
