...
func (s *Service) Register(ctx context.Context, x Qualification) (Qualification, error) {
	if x.State == "" {
		x.State = StateScheduled
	}
	now := s.clock.Now()
	if x.ReadyAt.IsZero() {
		x.ReadyAt = now
	...
	if err := s.policy.Validate(x); err != nil {
		return Qualification{}, platform.Wrap("register", "qualifications", x.ID, err)
	}
	saved, err := s.repo.Create(ctx, x)
	if err != nil {
		return Qualification{}, err
	}
	if err = s.events.Publish(ctx, platform.Event{Topic: "qualifications.registered", Key: saved.ID, Version: saved.Version, At: now, Attributes: platform.CloneMap(saved.Constraints)}); err != nil {
		_ = s.repo.Delete(context.Background(), saved.ID, saved.Version)
		return Qualification{}, platform.Wrap("publish", "qualifications", saved.ID, err)
	}
	return saved, nil
}
func (s *Service) Apply(ctx context.Context, cmd Command) (Qualification, error) {
	...
	if !s.policy.CanTransition(before.State, target) {
		return Qualification{}, platform.Wrap("transition", "qualifications", cmd.ID, platform.ErrInvalidTransition)
	}
	updated, err := s.repo.Update(ctx, cmd.ID, cmd.ExpectedVersion, func(next *Qualification) error {
		next.State = target
		if cmd.Priority != 0 {
	...
	if err = s.events.Publish(ctx, platform.Event{Topic: "qualifications.transitioned", Key: updated.ID, Version: updated.Version, At: updated.UpdatedAt, Attributes: attrs}); err != nil {
		_, rb := s.repo.Restore(context.Background(), before, updated.Version)
		if rb != nil {
			return Qualification{}, fmt.Errorf("publish: %w; rollback: %v", err, rb)
		}
		return Qualification{}, platform.Wrap("publish", "qualifications", updated.ID, err)
	}
	return updated, nil
}
...
func (s *Service) ReserveBest(ctx context.Context, fab, target string) (Candidate, Qualification, error) {
	ranked, err := s.Candidates(ctx, fab)
	if err != nil {
		return Candidate{}, Qualification{}, err
	}
	for _, c := range ranked {
		...
		if ctx.Err() != nil {
			return Candidate{}, Qualification{}, ctx.Err()
		}
	}
	return Candidate{}, Qualification{}, platform.Wrap("reserve", "qualifications", fab, platform.ErrCapacity)
}
