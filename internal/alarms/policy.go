...
func DefaultPolicy() Policy {
	return Policy{MaxQuantity: 1000, AllowedTransitions: map[string]map[string]bool{StateRaised: {StateAcknowledged: true}, StateAcknowledged: {StateCleared: true, StateRaised: true}, StateCleared: {StateRaised: true}}}
}
func (p Policy) Validate(x Alarm) error {
	if x.ID == "" || x.FabID == "" {
...
func (p Policy) Score(x Alarm, now time.Time) Candidate {
	reasons := []string{}
